...
### `kgcl_retro.chemistry.fg_instances`

Contextual functional-group occurrence matching:

- `FunctionalGroupInstance`: stores the functional-group name, SMARTS text, pattern index, matched atom indices, core atoms, KG embedding, product-computable descriptors, and null-token flag.
- `match_fg_instances()`: uses RDKit `GetSubstructMatches(..., uniquify=True)` to return one instance per unique occurrence, deduplicated by `(fg_name, sorted(match_atoms))`.
- `compute_fg_chem_descriptors()`: computes safe product-only descriptors such as core size, context boundary size, aromaticity, ring membership, formal charge, heteroatom count, and rough donor/acceptor flags.

This is used only by `fg_mode="contextual"`. Legacy mode still uses `match_fg()` and the original KGCL type-level attention behavior.
...
Contextual extension: `MolGraph` now accepts `fg_mode` with values `legacy`, `contextual`, and `none`. In contextual mode it preserves raw atom features, stores directed raw bond attributes, records matched functional-group instances, builds k-hop contextual subgraph metadata, and defers all trainable functional-group fusion to `kgcl_retro.models.contextual_fg`.

### `kgcl_retro.models.contextual_fg`

Trainable contextual functional-group modules:

- `ContextualFGGraphEncoder`: encodes each matched FG context subgraph from atom features, core membership, distance-to-core buckets, FG type ids, and bond features.
- `KGContextFusion`: gates contextual embeddings with the original KG embeddings and product-only descriptors.
- `AtomFGAttention`: lets atoms attend only to functional-group instances from the same molecule, with optional null token, core-membership bias, and distance bias.

This stage keeps the existing D-MPNN and edit heads. It does not implement sparse 2-FWL or expand the bond-edit action space.
