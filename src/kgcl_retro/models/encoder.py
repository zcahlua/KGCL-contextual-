...
class MPNEncoder(nn.Module):  # Explanation: defines MPNEncoder, directed message passing graph encoder
    ...
    def forward(
        ...
        atom_features_override: torch.Tensor = None,
        bond_attrs: torch.Tensor = None,
    ) -> torch.FloatTensor:  # Explanation: defines forward, which encode molecular graphs with D-MPNN and optional attention
        """
        ...
        mask: torch.Tensor,
            Masks on nodes
        """
        if hasattr(graph_tensors, "f_atoms"):
            f_atoms = graph_tensors.f_atoms
            f_bonds = graph_tensors.f_bonds
            a2b = graph_tensors.a2b
            b2a = graph_tensors.b2a
            b2revb = graph_tensors.b2revb
        else:
        ...
        if atom_features_override is not None:
            f_atoms = atom_features_override
            if bond_attrs is None:
                bond_attrs = graph_tensors.f_bond_attrs
            f_bonds = torch.cat([f_atoms[b2a], bond_attrs], dim=-1)
            f_bonds = f_bonds.clone()
            f_bonds[0] = 0
        # Input
