...
from kgcl_retro.cli.fg_options import add_fg_arguments, assert_processed_metadata, fg_config_from_args, processed_fg_subdir
...
def build_model_config(args):  # Explanation: defines build_model_config, which builds the KGCL model configuration
    ...
    model_config.update(fg_config_from_args(args))
    if model_config.get('fg_hidden_size') is None:
        model_config['fg_hidden_size'] = model_config['mpn_size']
    if model_config.get('fg_dropout') is None:
        model_config['fg_dropout'] = model_config['dropout_mpn']
    if model_config.get('fg_attn_dim') is None:
        model_config['fg_attn_dim'] = model_config['mpn_size']
...
def main(args):  # Explanation: defines main, which runs this script from command-line arguments
    ...
    fg_subdir = processed_fg_subdir(args)
    if fg_subdir:
        train_dir = os.path.join(train_dir, fg_subdir)
        assert_processed_metadata(train_dir, args)
...
def build_arg_parser():  # Explanation: constructs the training argument parser for scripts and console entry points.
    ...
    add_fg_arguments(parser, default_mode="legacy")
