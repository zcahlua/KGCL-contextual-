...
def resolve_eval_fg_config(checkpoint_config: dict[str, Any], args: Any) -> dict[str, Any]:
    ...
    for key in BROADER_ARCHITECTURE_KEYS:
        if key not in resolved:
            continue
        cli_value = args_dict.get(key)
        if cli_value is None:
            continue
        checkpoint_value = resolved[key]
        if checkpoint_value == cli_value:
            continue
        if not allow_override:
            raise ValueError(
                f"Evaluation architecture mismatch for {key}: checkpoint has {checkpoint_value!r}, "
                f"CLI requested {cli_value!r}. Omit the flag to use the checkpoint config, or pass "
                "--allow_architecture_override only when you intentionally load with a different architecture."
            )
        print(
            f"Warning: applying architecture override for {key}: checkpoint has "
            f"{checkpoint_value!r}, CLI requested {cli_value!r}."
        )
