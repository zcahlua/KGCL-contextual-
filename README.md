...
KGCL now supports three functional-group modes:

- `legacy`: the original KGCL behavior. SMARTS matches are reduced to functional-group types, KG embeddings are looked up, and non-trainable attention is applied in `MolGraph` before D-MPNN encoding. This is the default for backward compatibility.
- `contextual`: each SMARTS occurrence is kept as a matched instance with atom indices, expanded to a k-hop local molecular subgraph, encoded by a trainable contextual FG graph encoder, fused with the KG embedding, and injected through membership/distance-biased atom-to-FG attention inside the model.
- `none`: disables functional-group fusion for ablations.

`contextual_fg` is accepted as an alias for `contextual`.

Contextual mode requires regenerated prepared data because the saved tensor batches include matched-instance subgraph metadata:
